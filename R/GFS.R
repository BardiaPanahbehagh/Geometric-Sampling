...
#' @param fracsize GCD of all the inclusion probabilities
...
GFSfixedsize<-function( pik              #vector of inclusion probabilities
                       ,fracsize = NULL  #size of bars
                       ,entrop   = NULL  #increasing entropy, leads to decreasing zero second orders
                       ,Nentrop  = NULL  #number of times that we exchange the bars for increasing entropy
                       ,secondor = NULL  #1:shows results of second order
                       ,plot     = NULL  #1:plot
                       ,counter  = NULL  #1: active
                       ,round    = NULL  #1: round to decimal places of fracsize
)

{
  start.time = Sys.time()
  if(is.null(fracsize))fracsize<- 0.01
  if(is.null(entrop))  entrop  <- 1
  if(is.null(Nentrop)) Nentrop <- 10*length(pik)*(1/fracsize)
  ...
  for(i in 1:10){
    if(round(minmini, digits = i) > 0) {
      fracsizeo = min(fracsize, 1/10^i)
      break}
  }
  fracsize <- fracsizeo

  N<-length(pik)
  if(round == 1) pik = round(pik, decimalplaces(fracsize))
  pik[pik == 0] <- fracsize
  alpa<-1/(fracsize)#all possible sample
  alind<-array(0,alpa)
  xo<-ceiling(pik/fracsize)
  tre <- 0.0000001
  nn<-sum(pik)
  u<-c(1:N)
  ...
  #######################################################################
  ### Decreasing zero second order inclusions with increasing entropy ###
  #######################################################################
  while(entrop==1){
    if(counter == 1) pb3 <- txtProgressBar(min = 0,      # Minimum value of the progress bar
                                           max = Nentrop, # Maximum value of the progress bar
                                           style = 3,    # Progress bar style (also available style = 1 and style = 2)
                                           width = 50,   # Progress bar width. Defaults to getOption("width")
  ...
  df0 <- data.frame(
    ...
    "lower" = rep(seq(0,1-fracsize,fracsize),N),
    "upper" = rep(seq(fracsize,1,fracsize),N),
    "keep"  = c(Ma)
  )
  ...
  ### ready data for plot ###
  ##########################
  #row.names(Ma)<-seq(1-1/alpa,1/alpa-fracsize,-fracsize)
  while (plot==1) {
    if(entrop == 0) GGT <- "Madow version of GFS"
    if(entrop == 2) GGT <- "Poisson Sampling GFS"
    if(entrop == 1) GGT <- "Fixed Size, Reduced Zero SIPs GFS"
...
GFSoptimal<-function(    X                        #vector of inclusion probabilities
                        ,n                        #Sample size
                        ,inclusion = NULL         #1: unequal proportional to X, 2: equal
                        ,fracsize  = NULL         #size of bars
                        ,power     = NULL         #Number of tested designs,
                        ...
                        ,counter   = NULL         #1: active
                        ,round     = NULL         #1: round to decimal places of fracsize
                        ,shift     = NULL         #The shift to use just one variable in using variance of NHT in PPS
){
  start.time = Sys.time()
  if(is.null(fracsize))fracsize    <- 0.01
  if(is.null(inclusion))inclusion  <- 1
  if(is.null(power))power          <- 100
  ...
  N <- length(X)
  if(inclusion == 2) pik <- rep(n/N, N)
  if(inclusion == 1) pik <- inclusionprobabilities(X,n)
  X<-X+shift
  X[X<=0] <- fracsize
  minmini<-unique(min(pik))
  ...
  if(counter == 1) pb3 <- txtProgressBar(min = 0,      # Minimum value of the progress bar
                                         max = power*Nentrop, # Maximum value of the progress bar
                                         style = 3,    # Progress bar style (also available style = 1 and style = 2)
                                         width = 50,   # Progress bar width. Defaults to getOption("width")
                                         char = "2")   # Character used to create the bar
  ...
  for(b in 1:power){
    ...
    for(e in 1:Nentrop){
      if(counter == 1) setTxtProgressBar(pb3,e + (b-1)*Nentrop)
...

{
  ...
  if(is.null(fracsize)) fracsize    <- 0.01
  if(is.null(entrop))   entrop      <- 1
  if(is.null(Nentrop))  Nentrop     <- 100*length(pik)*(1/fracsize)
  if(is.null(counter))  counter     <- 1
  if(is.null(round))    round       <- 0
  if(is.null(plot))     plot        <- 0
  if(is.null(secondor)) secondor    <- 1
  ...
  E<-Nentrop
  H<-windows
  x<-round(pik, decimalplaces(fracsize))
  N<-length(x)
  alpa<-1/(fracsize)#all possible sample
  Ualpa<-c(1:alpa)
  u<-c(1:N)
  ...
  while(entrop==1){
    ...
    for(i in 1:llist){
      ...
      for(e in 1:round(Nentrop/llist)){
        if(counter == 1) setTxtProgressBar(pb3,(i-1)*round(Nentrop/llist)+e)
